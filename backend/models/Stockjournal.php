...
//    }
   public static function findProdrecId($no, $company_id, $branch_id){
    ...
    {
        $model = StockJournal::find()->where(['company_id'=>$company_id,'branch_id'=>$branch_id])->andFilterWhere(['date(trans_date)' => date('Y-m-d')])->andFilterWhere(['>','id',0])->andFilterWhere(['LIKE','journal_no','REP'])->andFilterWhere(['!=','production_type',28])->MAX('journal_no');
...
//        }
    ...
    public static function getLastNoReceiveTransfer($company_id,$branch_id)
    {
       // $model = StockJournal::find()->where(['company_id'=>$company_id,'branch_id'=>$branch_id])->andFilterWhere(['date(trans_date)' => date('Y-m-d')])->andFilterWhere(['>','id',19294])->andFilterWhere(['LIKE','journal_no','RTF'])->andFilterWhere(['!=','production_type',28])->MAX('journal_no');
        $model = StockJournal::find()->where(['company_id'=>$company_id,'branch_id'=>$branch_id])->andFilterWhere(['date(trans_date)' => date('Y-m-d')])->andFilterWhere(['>','id',0])->andFilterWhere(['LIKE','journal_no','RTF'])->andFilterWhere(['!=','production_type',28])->MAX('journal_no');
        // $model = StockJournal::find()->where(['company_id'=>$company_id,'branch_id'=>$branch_id])->andFilterWhere(['date(trans_date)' => date('Y-m-d')])->andFilterWhere(['!=','production_type',28])->MAX('journal_no');
        //   $model = Orders::find()->where(['date(order_date)' => date('Y-m-d', strtotime($date))])->MAX('order_no');
...
//        }
    ...
    {
        $model = StockJournal::find()->where(['company_id'=>$company_id,'branch_id'=>$branch_id])->andFilterWhere(['date(trans_date)' => date('Y-m-d')])->andFilterWhere(['>','id',0])->andFilterWhere(['=','production_type',15])->MAX('journal_no');
        // $model = StockJournal::find()->where(['company_id'=>$company_id,'branch_id'=>$branch_id])->andFilterWhere(['date(trans_date)' => date('Y-m-d')])->andFilterWhere(['!=','production_type',28])->MAX('journal_no');
        //   $model = Orders::find()->where(['date(order_date)' => date('Y-m-d', strtotime($date))])->MAX('order_no');

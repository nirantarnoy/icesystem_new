...
<div id="div1">
    ...
    if ($find_customer_id != null || $find_customer_id != '') {
        $modelx = \common\models\QueryUserDailyLogin::find()->select(['date(login_date)', 'user_id', 'second_user_id'])->where(['user_id' => $find_customer_id])
            ->andFilterWhere(['BETWEEN', 'login_date', $from_date, $to_date])
            ->groupBy(['date(login_date)', 'user_id'])->orderBy(['login_date0' => SORT_ASC])->all();
    }
    ?>

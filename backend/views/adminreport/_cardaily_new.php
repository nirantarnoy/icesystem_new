...
if($find_emp_id != null){
    $model_line = \common\models\QuerySaleMobileDataNew::find()->select(['route_id'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
        ->andFilterWhere(['company_id' => $company_id, 'branch_id' => $branch_id,'route_id' => $find_emp_id])->orderBy(['route_id' => SORT_ASC])->groupBy('route_id')->all();
}else{
    $model_line = \common\models\QuerySaleMobileDataNew::find()->select(['route_id'])->where(['BETWEEN', 'date(order_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
        ->andFilterWhere(['company_id' => $company_id, 'branch_id' => $branch_id])->orderBy(['route_id' => SORT_ASC])->groupBy('route_id')->all();
}

$print_type_data = [['id'=>1,'name'=>'ไม่แยกวันที่'],['id'=>2,'name'=>'แยกวันที่']];
...
<div class="row">
    <div class="col-lg-9">
        <form action="<?= \yii\helpers\Url::to(['adminreport/cardaily2'], true) ?>" method="post" id="form-search">
            <table class="table-header" style="width: 100%;font-size: 18px;" border="0">
                <tr>
                    ...
                    <td style="width: 25%; text-align: right">
                        <a href="<?= \yii\helpers\Url::to(['adminreport/cardaily'], true) ?>" class="btn btn-info">รายงานแบบรวม</a>
                    </td>
                </tr>
            </table>
        </form>
...
//              $total_all_line_qty = 0;
            ...
            ?>
            <tr>
                <td style="text-align: left;padding: 8px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= \backend\models\Deliveryroute::findName($value->route_id) ?></td>
                <td style="text-align: left;padding: 8px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $car_data != null ? $car_data[0]['car_name'] : '' ?></td>
                <td style="text-align: left;padding: 8px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $car_data != null ? $car_data[0]['emp_name'] : '' ?></td>
                <?php
                ...
                <?php for ($x = 0; $x <= count($product_header) - 1; $x++): ?>
                    ...
                    ?>
                    <td style="text-align: center;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $product_line_qty == 0 ? '-' : number_format($product_line_qty, 1) ?></td>
                <?php endfor; ?>
                <!--                <td style="text-align: center;padding: 0px;padding-right: 5px;border: 1px solid grey">-->
                <? //= number_format($line_qty) ?><!--</td>-->
                <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $product_line_free_qty == 0 ? '-' : number_format($product_line_free_qty, 1) ?></td>
                <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: yellow;font-weight: bold;"><?= number_format(($line_qty_total + $product_line_free_qty), 1) ?></td>
                <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $return_line_qty == 0 ? "-" : number_format($return_line_qty, 1) ?></td>
                <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $line_cash_amount_total == 0 ? "-" : number_format($line_cash_amount_total, 2) ?></td>
                <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $line_credit_amount_total == 0 ? "-" : number_format($line_credit_amount_total, 2) ?></td>
                <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $product_line_receive_cash == 0 ? "-" : number_format($product_line_receive_cash, 2) ?></td>
                <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $product_line_receive_transfer == 0 ? "-" : number_format($product_line_receive_transfer, 2) ?></td>
            ...
            $date_loop = $date1->diff($date2)->days + 1;

            $line_detail_qty_total = 0;
            $free_daily_qty_total = 0;
            $return_line_detail_qty_total = 0;
            $product_line_cash_amount_total = 0;
            $free_daily_qty = 0;
            $return_line_detail_qty = 0;

            ?>
            <?php for ($k = 0; $k <= $date_loop - 1; $k++): ?>
                <?php
//                $line_detail_qty_total = 0;
//                $free_daily_qty_total = 0;
//                $return_line_detail_qty_total = 0;
//                $product_line_cash_amount_total = 0;
                ...
                <tr>
                    ...
                    <td style="text-align: center;padding: 8px;padding-right: 5px;border: 1px solid grey"></td>
                    <td style="text-align: center;padding: 8px;padding-right: 5px;border: 1px solid grey"><?= date('d-m-Y', strtotime($from_date . '+' . $k . 'days')) ?></td>
                    <?php for ($z = 0; $z <= count($product_header) - 1; $z++): ?>
                        <?php
                        $product_line_detail_qty = getOrderQty2Detail($value->route_id, $product_header[$z], date('d-m-Y', strtotime($from_date . '+' . $k . 'days')));
                        $line_detail_qty_total = ($line_detail_qty_total + $product_line_detail_qty);

                        ?>
                        <td style="text-align: center;padding: 8px;padding-right: 5px;border: 1px solid grey"><?= $product_line_detail_qty == 0 ? '-' : number_format($product_line_detail_qty, 1) ?></td>
                    <?php endfor; ?>
                    <?php
                    $free_daily_qty = getFreeDaily($value->route_id, date('d-m-Y', strtotime($from_date . '+' . $k . 'days')));
                    //$free_daily_qty_total = ($free_daily_qty_total + $free_daily_qty);

                    $return_line_detail_qty = getReturncarQtyDaily($value->route_id, date('d-m-Y', strtotime($from_date . '+' . $k . 'days')));
                    //$return_line_detail_qty_total = ($return_line_detail_qty_total + $return_line_detail_qty);

                    $product_line_detail_cash_amount = getCashAmountDaily($value->route_id, date('d-m-Y', strtotime($from_date . '+' . $k . 'days')));

                    $product_line_detail_credit_amount = getCreditAmountDaily($value->route_id, date('d-m-Y', strtotime($from_date . '+' . $k . 'days')));

                    $product_line_detail_cash_receive_amount = getReceiveCashDaily($value->route_id, date('d-m-Y', strtotime($from_date . '+' . $k . 'days')));

                    $product_line_detail_transfer_amount = getReceiveTransferDaily($value->route_id, date('d-m-Y', strtotime($from_date . '+' . $k . 'days')));

                    $line_detail_total = ($product_line_detail_cash_amount + $product_line_detail_credit_amount + $product_line_detail_cash_receive_amount + $product_line_detail_transfer_amount);
                    ?>

                    <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: skyblue;font-weight: bold;"><?= $free_daily_qty == 0 ? "-" : number_format($free_daily_qty, 1) ?></td>
                    <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: yellow;font-weight: bold;"><?= $line_detail_qty_total == 0 ? "-" : number_format($line_detail_qty_total, 1) ?></td>
                    <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey"><?= $return_line_detail_qty == 0 ? "-" : number_format($return_line_detail_qty, 1) ?></td>
                    <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey"><?= $product_line_detail_cash_amount == 0 ? "-" : number_format($product_line_detail_cash_amount, 2) ?></td>
                    <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey"><?= $product_line_detail_credit_amount == 0 ? "-" : number_format($product_line_detail_credit_amount, 2) ?></td>
                    <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey"><?= $product_line_detail_cash_receive_amount == 0 ? "-" : number_format($product_line_detail_cash_receive_amount, 2) ?></td>
                    <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey"><?= $product_line_detail_transfer_amount == 0 ? "-" : number_format($product_line_detail_transfer_amount, 2) ?></td>
                    <td style="text-align: right;padding: 8px;padding-right: 5px;border: 1px solid grey;background-color: mediumseagreen;"><?=$line_detail_total == 0 ? "-" : number_format($line_detail_total, 2)?></td>
                </tr>
            <?php endfor; ?>
        <?php endforeach; ?>
        <tfoot>
...
function getOrderQty2Detail($route_id, $product_id, $line_date)
{
    $data = 0;
    if ($route_id && $product_id) {
        $model_qty = \common\models\TransactionCarSale::find()->select(['SUM(credit_qty) as credit_qty', 'SUM(cash_qty) as cash_qty'])->where(['date(trans_date)' => date('Y-m-d', strtotime($line_date))])
            ->andFilterWhere(['route_id' => $route_id, 'product_id' => $product_id])->groupBy(['product_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->credit_qty + $model_qty->cash_qty);
        }
...
function getFreeDaily($route_id, $line_date)
{
    $data = 0;
    if ($route_id) {
        $model_qty = \common\models\TransactionCarSale::find()->select(['SUM(free_qty) as free_qty'])->where(['date(trans_date)' => date('Y-m-d', strtotime($line_date))])
            ->andFilterWhere(['route_id' => $route_id])->groupBy(['route_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->free_qty);
        }
...
function getReturncarQtyDaily($route_id, $line_date)
{
    $data = 0;
//    if ($route_id) {
//        $model_qty = \common\models\TransactionCarSale::find()->select(['return_qty'])->where(['BETWEEN', 'date(trans_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
...
//    }
    $model_qty = \common\models\TransactionCarSaleRoutePay::find()
        ->where(['date(trans_date)' => date('Y-m-d', strtotime($line_date))])
        ->andFilterWhere(['route_id' => $route_id])->sum('return_car_qty');
    if ($model_qty != null) {
        $data = ($model_qty);
    }
...
function getCashAmountDaily($route_id, $line_date)
{
    $data = 0;
    if ($route_id) {
        $model_qty = \common\models\TransactionCarSale::find()->select(['SUM(cash_amount) as cash_amount'])->where(['date(trans_date)' => date('Y-m-d', strtotime($line_date))])
            ->andFilterWhere(['route_id' => $route_id])->groupBy(['route_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->cash_amount);
        }
...
function getCreditAmountDaily($route_id, $line_date)
{
    $data = 0;
    if ($route_id) {
        $model_qty = \common\models\TransactionCarSale::find()->select(['SUM(credit_amount) as credit_amount'])->where(['date(trans_date)' => date('Y-m-d', strtotime($line_date))])
            ->andFilterWhere(['route_id' => $route_id])->groupBy(['route_id'])->one();
        if ($model_qty != null) {
            $data = ($model_qty->credit_amount);
        }
...
function getReceiveCashDaily($route_id, $line_date)
{
    $data = 0;
    if ($route_id) {
//        $model_qty = \common\models\TransactionCarSale::find()->select(['receive_cashx'])->where(['BETWEEN', 'date(trans_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
...
function getReceiveTransferDaily($route_id, $line_date)
{
    $data = 0;
//    if ($route_id) {
//        $model_qty = \common\models\TransactionCarSale::find()->select(['receive_transter'])->where(['BETWEEN', 'date(trans_date)', date('Y-m-d', strtotime($from_date)), date('Y-m-d', strtotime($to_date))])
